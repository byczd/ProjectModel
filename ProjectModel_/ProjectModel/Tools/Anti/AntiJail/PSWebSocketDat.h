...
//3
//static __attribute__((always_inline)) int jailCydiaUrl(){
//    NSURL *FakeURL = [NSURL URLWithString:CYDIAPACKAGEURL];
...
//4
static __attribute__((always_inline)) int jailfstab() {
    @try {
...
//5
static __attribute__((always_inline)) int jailSymbolicLink(){
    @try {
...
//1-1
static __attribute__((always_inline)) int jailStatPath(){
        ...
        @try{
            ...
            if ((ret = dladdr(func_stat, &dylib_info))) {
                ...
                if (strcmp(dylib_info.dli_fname, "/usr/lib/system/libsystem_kernel.dylib")) {
                    //相等(为0)不一定是越狱，但不相等，则肯定是被攻击了=非0为=true=表示stat被hook攻击
...
//6
static __attribute__((always_inline)) int jailbreak(int iFlag){
    int flag=jailEnv();
